...
class Net():
    def __init__(self, pretrained = True, feature_extract = False, num_classes = 2, checkpoint = False): #features = tb positive/negative
        ...
        self.load_checkpoint(checkpoint, "optimizer")
    ...
    def load_checkpoint(self, checkpoint, string):
        #load model
        if checkpoint and string == "model":
            path_string = f"./checkpoints/{self.model_name}/model/*.pth"
        ...
        if checkpoint and string == "optimizer":
            path_string = f"./checkpoints/{self.model_name}/optimizer/*.pth"
    ...
    def load_model(self, pretrained, num_classes):
        if self.model_name == self.model_names[0]: #densenet
        ...
        elif self.model_name == self.model_names[1]: #resnet
            self.model = models.resnext101_32x8d(pretrained=pretrained)
            num_features = self.model.fc.in_features
            self.model.fc = nn.Linear(num_features,num_classes)
            self.last_layer_name = "fc"
        elif self.model_name == self.model_names[2]: #squeezenet
            #virker nok ikke.
            ...
            self.last_layer_name = "classifier"
        elif self.model_name == self.model_names[3]: #chexnet
            self.model = models.densenet121(pretrained=pretrained)
            print(self.model)
            num_features = self.model.classifier.in_features
            self.model.classifier = nn.Sequential(
            ...
            self.last_layer_name = "classifier"
        elif self.model_name == self.model_names[4]: #vgg19
            ...
            self.last_layer_name = "classifier"
        elif self.model_name == self.model_names[5]: #densenet201
        ...
        elif self.model_name == self.model_names[6]: #inception v3
        ...
        elif self.model_name == self.model_names[7]: #resnet
...
net=Net()
KfoldTrain(net, checkpoint_every=3)
